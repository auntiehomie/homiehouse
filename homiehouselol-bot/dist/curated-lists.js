...
export class CuratedListService {
    ...
    // Create a new list
    static async createList(fid, listName, description, isPublic = false) {
        try {
    ...
    // Add cast to a list
    static async addCastToList(listId, castHash, addedByFid, castData, notes) {
        try {
            const { data, error } = await supabase
    ...
    static async getListByName(fid, listName) {
        try {
            const { data, error } = await supabase
                .from('curated_lists')
                .select('*')
...
export class BotConversationService {
    ...
    static async getActiveConversation(userFid, conversationType) {
        try {
            let query = supabase
    ...
    static async startConversation(userFid, conversationType, state, contextData, parentCastHash) {
        try {
            const { data, error } = await supabase
                .from('bot_conversations')
                .insert([{
    ...
    static async updateConversation(conversationId, state, contextData) {
        try {
            const { data, error } = await supabase
                .from('bot_conversations')
                .update({
