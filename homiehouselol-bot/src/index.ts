...
async function handleMention(castText: string, parentCastHash: string): Promise<void> {
  ...
  try {
    ...
    const file = path.join(__dirname, '..', 'replied_casts.json');
    const casts: RepliedCast[] = Array.from(repliedCasts).map(h => ({ hash: h, timestamp: Date.now() }));
...
async function generateBotResponse(question: string, persona: string): Promise<string> {
