...
export async function GET(req: NextRequest) {
  ...
  if (!cast_id)
    return NextResponse.json({ error: "cast_id required" }, { status: 400 });
  ...
  try {
    ...
    const notes =
      await sql`SELECT * FROM cast_notes WHERE cast_id = ${cast_id} ORDER BY created_at ASC`;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { cast_id, note } = await req.json();
    if (!cast_id || !note)
      return NextResponse.json(
        { error: "cast_id and note required" },
        { status: 400 },
      );
    ...
    return handleApiError(err, "POST /cast-notes");
  }
}
