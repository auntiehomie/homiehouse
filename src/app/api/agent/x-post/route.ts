...
async function getRecentXPosts(limit = 8): Promise<XAgentPost[]> {
  try {
    ...
    console.warn(
      "[agent/x-post] getRecentXPosts failed:",
...
async function saveXPost(params: {
  xPostId?: string;
  text: string;
  source: string;
  topic?: string;
...
async function writeXPost(
  ...
  if (process.env.ANTHROPIC_API_KEY) {
    try {
      ...
      console.warn(
        "[agent/x-post] Anthropic post failed, using free providers:",
        err?.message,
      );
    }
  }
...
export async function GET(request: NextRequest) {
  try {
      ...
      if (casts.length > 1) {
        try {
          ...
          await saveXPost({
            ...
            text: firstText,
            source: chosen.mode,
            topic: kbArticle?.title?.slice(0, 80) || topic,
          });
          ...
          console.log(
            `[agent/x-post] Posted thread (${chosen.mode}): ${casts.length} tweets`,
          );
          return NextResponse.json({
            ok: true,
            mode: chosen.mode,
          ...
          console.warn(
            "[agent/x-post] Thread publish failed, falling back to single tweet:",
            threadErr?.message,
          );
    ...
    await saveXPost({
      ...
      text: content,
      source: chosen.mode,
      topic: topic || news?.headline?.slice(0, 80),
    });
    ...
    if (error?.message?.includes("X API not configured")) {
      console.log(
        "[agent/x-post] Not configured — skipping (this is expected until X credentials are provisioned)",
      );
