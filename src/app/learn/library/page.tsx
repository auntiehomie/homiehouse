...
export const metadata: Metadata = {
  ...
  description:
    "Free, practical lessons on wallet safety, scam defense, transaction approvals, risk budgets, and DeFi due diligence.",
  alternates: { canonical: "/learn/library" },
  openGraph: {
    title: "Crypto Safety Library | HomieHouse",
    description:
      "Learn how to participate in crypto without letting one mistake knock you out of the ecosystem.",
    url: `${BASE_URL}/learn/library`,
    images: [
      `${BASE_URL}/api/og/content?kind=learning%20library&title=Stay%20in%20the%20Game&description=Practical%20crypto%20safety%2C%20without%20the%20hype.`,
    ],
  },
  other: {
    ...
    <main className="mx-auto max-w-5xl px-4 py-10 sm:px-6 sm:py-16">
      ...
      <header className="max-w-3xl">
        <p className="mb-3 text-sm font-semibold uppercase tracking-[0.2em] text-[var(--accent)]">
          Free safety curriculum
        </p>
        <h1 className="text-4xl font-bold tracking-tight sm:text-6xl">
          Stay in the game.
        </h1>
        <p className="mt-5 text-lg leading-8 text-[var(--muted-on-dark)]">
          Learn how wallets, signatures, scams, position sizing, and DeFi risk
          actually work. No price calls. No guaranteed returns. No wallet
          connection required to read.
        </p>
      ...
      <section
        ...
        aria-label="Safety lessons"
      >
        {SAFETY_CURRICULUM.map(({ module }, index) => (
          <Link
            key={module.id}
            href={`/learn/library/${module.id}`}
            className="group rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 no-underline transition hover:border-[var(--accent)]"
          >
            ...
            <h2 className="mt-4 text-xl font-bold text-[var(--text-on-dark)] group-hover:text-[var(--accent)]">
              {module.title}
            </h2>
      ...
      <aside className="mt-10 rounded-2xl border border-amber-400/30 bg-amber-400/10 p-5 text-sm leading-6 text-[var(--muted-on-dark)]">
        HomieHouse provides education, not individualized financial, tax, or
        legal advice. Crypto assets and protocols can lose value or fail; never
