...
export default function NotificationsPage() {
  ...
  const getActor = (notification: Notification): Actor | null =>
    notification.actor || notification.user || notification.cast?.author || null;
        ...
        <div className="flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
          {(['all', 'likes', 'recasts', 'replies', 'follows', 'mentions'] as NotificationFilter[]).map((filterType) => (
            <button
              ...
              style={
                filter === filterType
                  ? { background: 'var(--btn-primary-bg)', color: 'var(--btn-primary-color)', padding: '6px 14px', borderRadius: 8, fontSize: 13, fontWeight: 500, whiteSpace: 'nowrap', border: 'none', cursor: 'pointer', flexShrink: 0 }
                  : { background: 'var(--surface)', color: 'var(--muted-on-dark)', padding: '6px 14px', borderRadius: 8, fontSize: 13, fontWeight: 500, whiteSpace: 'nowrap', border: '1px solid var(--border)', cursor: 'pointer', flexShrink: 0 }
              }
            >
      ...
      <div>
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '0.35rem', marginBottom: 4 }}>
                      ...
                      <span style={{ fontSize: 13, color: 'var(--muted-on-dark)' }}>{notifText}</span>
                    </div>
        ...
        {hasMore && cursor && (
          <div style={{ display: 'flex', justifyContent: 'center', marginTop: '1.5rem' }}>
            <button onClick={() => console.log('Load more with cursor:', cursor)} className="btn primary">
              Load More
            </button>
          </div>
        )}
