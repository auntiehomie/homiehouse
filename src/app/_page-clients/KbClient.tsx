...
export default function KbClient() {
  ...
  const fetchCasts = useCallback(async () => {
    ...
    try {
      ...
      if (!auth) return;
      const res = await fetch(`/api/saved-casts`, { headers: { ...auth } });
  ...
  async function saveNote(castId: string) {
    ...
    try {
      const res = await fetch("/api/cast-notes", {
        ...
        body: JSON.stringify({ cast_id: castId, note: noteText.trim() }),
      });
    ...
    return (
      <div className="min-h-screen bg-white dark:bg-black flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <Link
            href="/"
            className="inline-block text-sm text-zinc-400 hover:underline"
          >
            ← Back to feed
          </Link>
        </div>
        ...
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-zinc-400 truncate">
              {loading
                ? "Loading…"
                : `${casts.length} saved cast${casts.length !== 1 ? "s" : ""}`}
            </p>
          </div>
        </div>
      ...
      <main className="max-w-2xl mx-auto px-4 py-4 space-y-3 pb-20">
        ...
        {loading && (
          <div className="space-y-3">
            {[...Array(4)].map((_, i) => (
              <div
                key={i}
                className="animate-pulse rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 p-4 space-y-2"
              >
        ...
        {error && (
          <div className="rounded-2xl border border-red-200 dark:border-red-900 bg-red-50 dark:bg-red-950/30 p-4 text-sm text-red-600 dark:text-red-400">
            ⚠️ {error}
            <button onClick={fetchCasts} className="ml-3 underline">
              Retry
            </button>
          </div>
        )}
        ...
        {!loading && !error && casts.length === 0 && (
          <div className="text-center py-20 space-y-3">
            ...
            <Link
              href="/"
              className="inline-block mt-4 px-5 py-2 bg-zinc-900 dark:bg-white text-white dark:text-black rounded-xl text-sm font-semibold hover:opacity-80 transition-opacity"
            >
              Browse Feed
            </Link>
          </div>
        ...
        {!loading && !error && casts.length > 0 && visible.length === 0 && (
          <div className="text-center py-12 text-zinc-400">
            ...
            <button
              ...
              className="mt-3 text-xs text-zinc-400 hover:underline"
            >
              Clear filters
            </button>
          </div>
